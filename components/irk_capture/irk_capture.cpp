...
Readers retry the read once if a concurrent write is detected. This does not change behavior or timing.
*/
...
struct TimingConfig {
    ...
    static constexpr uint32_t HR_NOTIFY_INTERVAL_MS = 1000;        // Heart rate notification interval (keeps connection alive)
    static constexpr uint32_t ENC_TO_FIRST_TRY_DELAY_MS = 1000;    // Delay from encryption to first IRK poll (allows NVS write to complete)
    static constexpr uint32_t ENC_TRY_INTERVAL_MS = 1000;          // Interval between IRK polling attempts while connected
    static constexpr uint32_t ENC_GIVE_UP_AFTER_MS = 45000;        // Maximum time to poll for IRK before giving up
    static constexpr uint32_t POST_DISC_DELAY_MS = 800;            // Delay after disconnect for deferred IRK check (allows NVS flush)
    static constexpr uint32_t ENC_LATE_READ_DELAY_MS = 5000;       // Extended delay for IRK check after failed encryption
    static constexpr uint32_t IRK_MIN_POLL_INTERVAL_MS = 200;      // Minimum time between IRK polling attempts (prevents spam)
    ...
    static constexpr uint32_t MIN_REPUBLISH_INTERVAL_MS = 60000;   // Min time between republishing same IRK (60s)
};
...
static void log_conn_desc(uint16_t conn_handle) {
    ...
    if (ble_gap_conn_find(conn_handle, &d) == 0) {
        ...
        // Note: NimBLE does not expose a separate connection timeout distinct from supervision_timeout here;
        // we log supervision_timeout as provided.
        ESP_LOGD(TAG, "conn params: interval=%u latency=%u supervision_timeout=%u",
...
bool IRKCaptureComponent::should_publish_irk(const std::string &irk_hex, const std::string &addr) {
    ...
    if (irk_cache_.size() >= 10) {
        // Evict oldest (FIFO). Note: documented behavior; intentional to cap memory and keep UX predictable.
 ...
 * @param context_tag Context label for logging (e.g., "ENC_IMMEDIATE", "DISC_IMMEDIATE", "POLL_CONNECTED")
 *
...
static struct ble_gatt_chr_def devinfo_chrs[] = {
    ...
    },
    {
        ...
        // Reads can occur concurrently; there is a small window where a stale pointer could be seen.
        // This is acceptable in practice and does not affect pairing logic or timing.
...
static int chr_read_devinfo(uint16_t conn_handle, uint16_t, struct ble_gatt_access_ctxt *ctxt, void *arg) {
    ...
    // The pointer is refreshed on start_advertising() and update_ble_name(). Minor race is acceptable.
...
void IRKCaptureComponent::setup_ble() {
    ...
    // NOTE: All errors are logged; cleanup is guarded to avoid handle leaks. This does not change behavior.
